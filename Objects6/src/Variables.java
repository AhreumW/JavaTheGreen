...
public class Variables {//클래스 영역
	...
	1. 인스턴스 변수
	클래스 영역에 선언되며, 클래스의 인스턴스를 생성할 때 만들어진다(new). 그렇기 때문에
	...
	그래서 클래스 변수를 공유 변수(shared variable)이라고도 한다. 
	한 클래스의 모든 인턴스들이 공통적인 값을 유지해야하는 속성의 경우, 클래스 변수로 선언해야 한다. 
	인스턴스 변수는 인스턴스를 생성한 후에야 사용가능하지만, 클래스 변수는 인스턴스를 생성하지 않고도 
	언제라도 바로 사용할 수 있다는 특징이 있으며, '클래스이름.클래스변수'와 같은 형식으로 사용한다. 
	클래스가 로딩될 때 생성되어 프로그램이 종료될 때까지 유지된다. (lifecycle 생명주기)

...
/*	
	...
	컴파일 에러(compile-time error)?
			컴파일 할 때 발생하는 에러. / 코드 작성만 해도 나오는 에러(바로에러출력)
	런타임 에러(runtime error)?
			프로그램의 실행도중 발생하는 에러.	/메인에서 결정, 수행의 순간 결정.(중간에 에러출력)
	논리적 에러(logical error)?			
			컴파일도 잘 되고 실행도 잘 되지만 의도한 것과 다르게 동작하는 것.	/잘못된 공식
	(컴파일을 에러없이 성공적으로 마쳤다고 해서 프로그램의 실행 시에도 
			에러가 발생하지 않는 것은 아니다.)		
*//*	
	자바에서는 실행 시 발생할 수 있는 프로그램 오류를 에러(error)와 예외(exception) 두 가지로 구분하였다.
	에러?
			프로그램 코드에 의해서 수습될 수 없는 심각한 오류	(물리적,외부)
	예외?
...
//	try ~ catch()문
	public static void main(String[] args) {
		...
		for(int i =0; i<10; i++) {
			try {
				result = number / (int)(Math.random() * 10);	//나누기/0 error
